...
public final class CarvingPartSelectionMenu extends AbstractContainerMenu {
    private final Level level; private final BlockPos pos; private final List<ResourceLocation> candidates;
    ...
    public CarvingPartSelectionMenu(int id, Inventory inventory, BlockPos pos, List<ResourceLocation> candidates) {
    ...
    public List<ResourceLocation> candidates() { return candidates; }
    @Override public boolean clickMenuButton(Player player, int buttonId) {
        ...
        if (table == null || CarvingGameService.start(table, serverPlayer, candidates.get(buttonId)).isEmpty()) return false;
    ...
    public static void open(ServerPlayer player, BlockPos pos, List<NonMetalPartDefinition> definitions) {
        List<ResourceLocation> ids = definitions.stream().map(NonMetalPartDefinition::id).toList();
    ...
    private static void write(FriendlyByteBuf buffer, BlockPos pos, List<ResourceLocation> ids) {
        buffer.writeBlockPos(pos); buffer.writeVarInt(ids.size()); ids.forEach(buffer::writeResourceLocation);
    }
    private static List<ResourceLocation> readIds(FriendlyByteBuf buffer) {
        int size = Math.min(buffer.readVarInt(), 256); List<ResourceLocation> ids = new ArrayList<>(size);
        for (int i = 0; i < size; i++) ids.add(buffer.readResourceLocation()); return ids;
    }
}
