...
public enum FoodProcessingStation {
    ...
    public static FoodProcessingStation fromBlock(Block block) {
        if (block == Craftbound.HAND_MILL.get()) return HAND_MILL;
        if (block == Craftbound.DRYING_RACK.get()) return DRYING_RACK;
        if (block == Craftbound.COOKING_POT.get()) return COOKING_POT;
